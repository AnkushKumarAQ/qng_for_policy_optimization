...
def env_log_probs_and_rewards(env, policy, initial_state, inference_mode=False):
    """Plays one episode and collects log probabilities and rewards."""
    state = initial_state
    log_probs, rewards = [], []
    done = False
    ...
    while not done:
        ...
        # Inference mode takes the absolute best action; training mode samples probabilistically
...
def compute_reinforce_returns(rewards, gamma=0.99):
    """Calculates discounted returns and normalizes them across the batch."""
    returns = []
    G = 0
    ...
    # Batch normalization acts as our baseline to stabilize training
    if len(returns) > 1:
...
def evaluate_policy(env, policy, eval_seeds):
    """
    Tests the policy deterministically (inference mode) over a specific list of seeds.
